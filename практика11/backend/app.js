...
function findUserOr404(userId, res) {
...
app.put("/api/users/:id", authMiddleware, roleMiddleware(["admin"]),(req, res) => {
    ...
    if (req.body?.email === undefined && req.body?.first_name === undefined && req.body?.last_name === undefined && req.body?.role === undefined) {
        return res.status(400).json({
            error: "Нет данных для обновления",
        });
    }
...
*     responses:
*       204:
*         description: Польщователь успешно удален (нет тела ответа)
*       404:
*         description: Пользователь не найден
*/
